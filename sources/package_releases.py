...
class NpmPackageReleasesAdapter:
    ...
    def discover(self, ctx: RunContext, config: ScraperConfig) -> list[SourceRecord]:
        ...
        for query in input_value.source.get("search_queries", []):
            ...
            for obj in response.json().get("objects", []):
                ...
                if record is None:
                    continue
                if _safe_int(record.metrics.get("downloads_weekly")) < min_weekly_downloads:
                    continue
    ...
    def _record_from_package(
        ...
        weekly_downloads = self._weekly_downloads(package_name)
        return SourceRecord(
            ...
            author_id=str((version_data.get("author") or data.get("author") or {}).get("name") or ""),
            source_published_date=published_at,
        )

    def _weekly_downloads(self, package_name: str) -> int:
        try:
            response = http_get(
                f"{NPM_DOWNLOADS_URL}/{_npm_package_path(package_name)}",
                timeout=15,
                headers={"User-Agent": "octopus-crawler/0.1"},
            )
...
class PyPIPackageReleasesAdapter:
    spec = ChannelSpec(
        ...
        default_input=default_input(
            ...
            fetch={"window_days": 14, "limit": 3},
            filters={"skip_prerelease": True, "skip_yanked": True},
        ),
        description="抓取 PyPI watched packages 的近期 release；PyPI 官方 JSON 不提供可靠下载量。",
    )
